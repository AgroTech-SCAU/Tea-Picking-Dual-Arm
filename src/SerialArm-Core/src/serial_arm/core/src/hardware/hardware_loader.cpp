...
public:
    LoadedMotorBus(void* handle, MotorBus* bus, DestroyMotorBusFn destroy) noexcept
        : handle_(handle), bus_(bus), destroy_(destroy) {}

    ~LoadedMotorBus() override {
        if(bus_ && destroy_) destroy_(std::exchange(bus_, nullptr));
        if(handle_) dlclose(std::exchange(handle_, nullptr));
    }
    ...
    tl::expected<void, MotorBusErr> configure(const std::string& config_path) override { return bus_->configure(config_path); }
    tl::expected<void, MotorBusErr> connect() override { return bus_->connect(); }
    tl::expected<ActuatorState, MotorBusErr> read() override { return bus_->read(); }
    tl::expected<void, MotorBusErr> activate() override { return bus_->activate(); }
    tl::expected<void, MotorBusErr> write(const ActuatorCtrlCmd& cmd) override { return bus_->write(cmd); }
    tl::expected<void, MotorBusErr> stop() override { return bus_->stop(); }
    tl::expected<void, MotorBusErr> deactivate() override { return bus_->deactivate(); }
    tl::expected<void, MotorBusErr> recover() override { return bus_->recover(); }
    const HardwareCapabilities& capabilities() const noexcept override { return bus_->capabilities(); }
    void cleanup() noexcept override { bus_->cleanup(); }
    std::size_t size() const noexcept override { return bus_->size(); }
...
tl::expected<std::unique_ptr<MotorBus>, HardwareLoaderErr> HardwareLoader::load(
    ...
    if(plugin.find('/') == std::string::npos) candidates.push_back("lib" + plugin + ".so");
