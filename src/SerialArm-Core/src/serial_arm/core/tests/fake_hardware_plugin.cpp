...
public:
    tl::expected<void, serial_arm::MotorBusErr> configure(const std::string& config_path) override {
        try {
            ...
            return {};
        } catch(const YAML::Exception&) { return tl::make_unexpected(serial_arm::MotorBusErr::INVALID_CFG); }
    }
...
extern "C" serial_arm::MotorBus* create_motor_bus() { return new FakeHardwareBus(); }
extern "C" void destroy_motor_bus(serial_arm::MotorBus* bus) { delete bus; }
