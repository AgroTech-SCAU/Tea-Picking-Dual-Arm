...
TEST(HardwareOverrideTests, NoOverrideKeepsYamlValues) { EXPECT_EQ(load_name(), "main_can|/dev/ttyACM0|921600"); }
TEST(HardwareOverrideTests, SerialPortOverrideKeepsOtherYamlValues) {
    serial_arm::HardwareConfigOverrides overrides; overrides.serial_port = "/dev/ttyACM1";
...
TEST(HardwareOverrideTests, MultipleOverridesKeepUnspecifiedYamlValues) {
    serial_arm::HardwareConfigOverrides overrides; overrides.serial_port = "/dev/ttyACM1"; overrides.baudrate = 1000000;
    EXPECT_EQ(load_name(overrides), "main_can|/dev/ttyACM1|1000000");
}
TEST(HardwareOverrideTests, BusOverrideDoesNotAffectOtherFields) {
    serial_arm::HardwareConfigOverrides overrides; overrides.bus = "secondary_can";
...
TEST(HardwareOverrideTests, InvalidBaudrateOverrideIsRejected) {
    ...
    serial_arm::HardwareConfigOverrides overrides; overrides.baudrate = 0;
    serial_arm::HardwareLoader loader;
    auto bus = loader.load(SERIAL_ARM_TEST_FAKE_HARDWARE_PLUGIN, config.string(), overrides);
    ASSERT_FALSE(bus);
